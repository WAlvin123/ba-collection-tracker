...
// TODO: ownedFilter == false case and neutral case for both filter functions
  ...
  const handleNotPulled = () => {
    ...
    if (ownedFilter == true) {
      ...
      if (dmgFilter == false && schoolFilter == false && notOwnedFilter == false) { // ownedFilter only
        ...
        setFilteredCharacters(characters.filter(character => {
          if (schools.includes(character.school)) {
            return true
          } else return false
        }))
        setNotOwnedFilter(false)
        ...
        setFilteredCharacters(characters.filter(character => {
          if (dmgTypes.includes(character.damageType)) {
            return true
          } else return false
        }))
        setNotOwnedFilter(false)
        ...
        setFilteredCharacters(characters.filter(character => {
          if (dmgTypes.includes(character.damageType) && schools.includes(character.school)) {
            return true
          } else return false
        }))
        setNotOwnedFilter(false)
